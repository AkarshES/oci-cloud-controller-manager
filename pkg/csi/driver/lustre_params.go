...
func extractLustreStorageClassParameters(
	...
	if ad, ok := parameters["availabilityDomain"]; ok && strings.TrimSpace(ad) != "" {
		...
		if identityClient != nil {
			...
			} else if full != nil && full.Name != nil && *full.Name != "" {
				params.AvailabilityDomain = *full.Name
			} else {
				params.AvailabilityDomain = adTrim
			}
		} else {
			params.AvailabilityDomain = adTrim
		}
...
func deriveDefaultFSName(volumeName string) string {
	if volumeName == "" {
		return "lustrefs" //Just having default here, but volume name will not be empty as we check that in CreateVolume
	}
	// collect allowed chars from the end
	allowed := []rune{}
	for i := len(volumeName) - 1; i >= 0 && len(allowed) < 8; i-- {
	...
	if err := validateLustreFileSystemName(fs); err != nil {
		return "lustrefs" //This is not expected to happen, as csi volume names are uuids
	}
	return fs
}
