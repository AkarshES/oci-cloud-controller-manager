...
const (
	...
	maxVolumeAttachDetachErrorMsgBytes = 900
...
func (d *BlockVolumeControllerDriver) CreateVolume(ctx context.Context, req *csi.CreateVolumeRequest) (*csi.CreateVolumeResponse, error) {
	...
	if err != nil {
		...
		metricDimension = util.GetComponentForMetricDimension(util.ErrValidation, util.CSIStorageType)
	...
	if availableDomainShortName == "" {
		metricDimension = util.GetComponentForMetricDimension(util.ErrValidation, metricType)
		dimensionsMap[metrics.ComponentDimension] = metricDimension
	...
	if err != nil {
		...
		metricDimension = util.GetComponentForMetricDimension(errorType, metricType)
		dimensionsMap[metrics.ComponentDimension] = metricDimension
		metrics.SendMetricData(d.metricPusher, metric, time.Since(startTime).Seconds(), dimensionsMap)
	...
	metricDimension = util.GetComponentForMetricDimension(util.Success, metricType)
	dimensionsMap[metrics.ComponentDimension] = metricDimension
...
func (d *BlockVolumeControllerDriver) DeleteVolume(ctx context.Context, req *csi.DeleteVolumeRequest) (*csi.DeleteVolumeResponse, error) {
	...
	if req.VolumeId == "" {
		...
		csiMetricDimension = util.GetComponentForMetricDimension(util.ErrValidation, util.CSIStorageType)
...
func (d *BlockVolumeControllerDriver) ControllerPublishVolume(ctx context.Context, req *csi.ControllerPublishVolumeRequest) (*csi.ControllerPublishVolumeResponse, error) {
	...
	if err != nil {
		...
		return nil, status.Errorf(codes.InvalidArgument, "failed to get ProviderID by nodeName. error : %s", err.Error())
	}
	...
	if err != nil {
		...
		dimensionsMap[metrics.ComponentDimension] = csiMetricDimension
		metrics.SendMetricData(d.metricPusher, csiMetricPrefix, time.Since(startTime).Seconds(), dimensionsMap)
		return nil, status.Errorf(codes.Unknown, "failed to get the attachment options. error : %s", csi_util.TruncateError(err, maxVolumeAttachDetachErrorMsgBytes))
	}
	...
	if err != nil {
		...
		dimensionsMap[metrics.ComponentDimension] = csiMetricDimension
		metrics.SendMetricData(d.metricPusher, csiMetricPrefix, time.Since(startTime).Seconds(), dimensionsMap)
		return nil, status.Errorf(codes.Unknown, "failed to get compartmentID from node annotation. error : %s", csi_util.TruncateErrorOld(err.Error(), maxVolumeAttachDetachErrorMsgBytes))
	}
	...
	if err != nil && !client.IsNotFound(err) {
		...
		dimensionsMap[metrics.ComponentDimension] = csiMetricDimension
		metrics.SendMetricData(d.metricPusher, csiMetricPrefix, time.Since(startTime).Seconds(), dimensionsMap)
		return nil, status.Errorf(codes.Internal, "Failed to list volume attachments :  %s", csi_util.TruncateErrorOld(err.Error(), maxVolumeAttachDetachErrorMsgBytes))
	}
	...
	if err == nil && len(volumeAttachments) > 0 {
		...
		if !volumeAttachmentOptions.isShareable && volumeAttachments[0].GetLifecycleState() == core.VolumeAttachmentLifecycleStateDetaching {
			...
			if err != nil {
				...
				dimensionsMap[metrics.ComponentDimension] = csiMetricDimension
				metrics.SendMetricData(d.metricPusher, csiMetricPrefix, time.Since(startTime).Seconds(), dimensionsMap)
				return nil, status.Errorf(codes.Internal, "Error while waiting for volume to get detached before attaching: %s", csi_util.TruncateErrorOld(err.Error(), maxVolumeAttachDetachErrorMsgBytes))
			}
		} else {
			...
			if nodeVolumeAttachment != nil {
				...
				if nodeVolumeAttachment.GetLifecycleState() == core.VolumeAttachmentLifecycleStateAttaching {
					...
					if err != nil {
						...
						dimensionsMap[metrics.ComponentDimension] = csiMetricDimension
						metrics.SendMetricData(d.metricPusher, csiMetricPrefix, time.Since(startTime).Seconds(), dimensionsMap)
						return nil, status.Errorf(codes.Internal, "Failed to attach volume to the node: %s", csi_util.TruncateErrorOld(err.Error(), maxVolumeAttachDetachErrorMsgBytes))
					}
				}
				...
				if nodeVolumeAttachment.GetLifecycleState() == core.VolumeAttachmentLifecycleStateAttached {
					...
					if err != nil {
						...
						dimensionsMap[metrics.ComponentDimension] = csiMetricDimension
						metrics.SendMetricData(d.metricPusher, csiMetricPrefix, time.Since(startTime).Seconds(), dimensionsMap)
						return nil, status.Errorf(codes.Internal, "Failed to generate publish context: %s", csi_util.TruncateErrorOld(err.Error(), maxVolumeAttachDetachErrorMsgBytes))
					}
					return resp, nil
				}
			}
	...
	if volumeAttachmentOptions.useParavirtualizedAttachment {
		...
		if err != nil {
			...
			log.With("service", "compute", "verb", "create", "resource", "volumeAttachment", "statusCode", util.GetHttpStatusCode(err)).
				With("instanceID", id).Infof("5. Truncated failed paravirtualized attachment instance to volume. error : %s", csi_util.TruncateErrorOld(err.Error(), maxVolumeAttachDetachErrorMsgBytes))

			return nil, status.Errorf(codes.Internal, "failed paravirtualized attachment instance to volume. error : %s",  csi_util.TruncateError(err, maxVolumeAttachDetachErrorMsgBytes))
		}
	} else {
		...
		if err != nil {
			...
			dimensionsMap[metrics.ComponentDimension] = csiMetricDimension
			metrics.SendMetricData(d.metricPusher, csiMetricPrefix, time.Since(startTime).Seconds(), dimensionsMap)
			return nil, status.Errorf(codes.Internal, "failed iscsi attachment instance to volume :  %s", csi_util.TruncateErrorOld(err.Error(), maxVolumeAttachDetachErrorMsgBytes))
		}
	}
...
func (d *BlockVolumeControllerDriver) ControllerUnpublishVolume(ctx context.Context, req *csi.ControllerUnpublishVolumeRequest) (*csi.ControllerUnpublishVolumeResponse, error) {
	...
	if err != nil {
		if !client.IsNotFound(err) {
			...
			dimensionsMap[metrics.ComponentDimension] = csiMetricDimension
			metrics.SendMetricData(d.metricPusher, csiMetricPrefix, time.Since(startTime).Seconds(), dimensionsMap)
			return nil, status.Errorf(codes.Internal, "Failed to find volume attachment : %s", csi_util.TruncateErrorOld(err.Error(), maxVolumeAttachDetachErrorMsgBytes))
		}
	...
	if attachedVolume.GetLifecycleState() != core.VolumeAttachmentLifecycleStateDetaching {
		...
		if err != nil {
			...
			dimensionsMap[metrics.ComponentDimension] = csiMetricDimension
			metrics.SendMetricData(d.metricPusher, csiMetricPrefix, time.Since(startTime).Seconds(), dimensionsMap)
			return nil, status.Errorf(codes.Unknown, "volume can not be detached %s", csi_util.TruncateErrorOld(err.Error(), maxVolumeAttachDetachErrorMsgBytes))
		}
	}
	...
	if err != nil {
		...
		dimensionsMap[metrics.ComponentDimension] = csiMetricDimension
		metrics.SendMetricData(d.metricPusher, csiMetricPrefix, time.Since(startTime).Seconds(), dimensionsMap)
		return nil, status.Errorf(codes.Unknown, "timed out waiting for volume to be detached %s", csi_util.TruncateErrorOld(err.Error(), maxVolumeAttachDetachErrorMsgBytes))
	}
...
func (d *BlockVolumeControllerDriver) CreateSnapshot(ctx context.Context, req *csi.CreateSnapshotRequest) (*csi.CreateSnapshotResponse, error) {
	...
	if req.Name == "" {
		...
		snapshotMetricDimension = util.GetComponentForMetricDimension(util.ErrValidation, util.CSIStorageType)
	...
	if err != nil {
		errorType = util.GetError(err)
		snapshotMetricDimension = util.GetComponentForMetricDimension(errorType, util.CSIStorageType)
		dimensionsMap[metrics.ComponentDimension] = snapshotMetricDimension
		metrics.SendMetricData(d.metricPusher, metrics.BlockSnapshotProvision, time.Since(startTime).Seconds(), dimensionsMap)
	...
	if len(snapshots) > 0 {
		...
		if err != nil {
			if strings.Contains(err.Error(), "timed out") {
				...
				snapshotMetricDimension = util.GetComponentForMetricDimension(util.BackupCreating, util.CSIStorageType)
				dimensionsMap[metrics.ComponentDimension] = snapshotMetricDimension
		...
		snapshotMetricDimension = util.GetComponentForMetricDimension(util.Success, util.CSIStorageType)
		dimensionsMap[metrics.ComponentDimension] = snapshotMetricDimension
		metrics.SendMetricData(d.metricPusher, metrics.BlockSnapshotProvision, time.Since(snapshot.TimeRequestReceived.Time).Seconds(), dimensionsMap)
