...
func (d *LustreControllerDriver) CreateVolume(ctx context.Context, req *csi.CreateVolumeRequest) (*csi.CreateVolumeResponse, error) {
	...
	// If AD is not provided, try to resolve from topology (preferred, then requisite)
	if sc.AvailabilityDomain == "" && req.GetAccessibilityRequirements() != nil {
		findAD := func(topology []*csi.Topology) string {
			for _, t := range topology {
				if t == nil || t.Segments == nil {
					continue
				}
				seg := t.Segments
				if v, ok := seg[kubeAPI.LabelTopologyZone]; ok && v != "" {
					return v
				}
				if v, ok := seg[kubeAPI.LabelZoneFailureDomain]; ok && v != "" {
					return v
				}
			}
			return ""
		...
		if ad := findAD(req.GetAccessibilityRequirements().Preferred); ad != "" {
			fullAd, err := identityClient.GetAvailabilityDomainByName(ctx, sc.CompartmentId, ad)
			if err != nil || fullAd == nil {
				log.With("service", "identity", "verb", "list", "resource", "availabilityDomain", "statusCode", util.GetHttpStatusCode(err)).
					With(zap.Error(err)).Errorf("Invalid availabilityDomain: %s (from preferred topology) for compartment: %s", ad, sc.CompartmentId)
				metricDimensions[metrics.ComponentDimension] = util.GetComponentForMetricDimension(util.GetError(err), util.CSIStorageType)
				metrics.SendMetricData(d.metricPusher, metrics.LustreProvision, time.Since(startTime).Seconds(), metricDimensions)
				return nil, status.Errorf(codes.InvalidArgument, "Invalid availabilityDomain: %s for compartment %s, error: %v", ad, sc.CompartmentId, err)
			}
			sc.AvailabilityDomain = *fullAd.Name
			log.Info("AD is not provided in storage class and determined from Preferred topology.")
		} else if ad := findAD(req.GetAccessibilityRequirements().Requisite); ad != "" {
			fullAd, err := identityClient.GetAvailabilityDomainByName(ctx, sc.CompartmentId, ad)
			if err != nil || fullAd == nil {
				log.With("service", "identity", "verb", "list", "resource", "availabilityDomain", "statusCode", util.GetHttpStatusCode(err)).
					With(zap.Error(err)).Errorf("Invalid availabilityDomain: %s (from requisite topology) for compartment: %s", ad, sc.CompartmentId)
				metricDimensions[metrics.ComponentDimension] = util.GetComponentForMetricDimension(util.GetError(err), util.CSIStorageType)
				metrics.SendMetricData(d.metricPusher, metrics.LustreProvision, time.Since(startTime).Seconds(), metricDimensions)
				return nil, status.Errorf(codes.InvalidArgument, "Invalid availabilityDomain: %s for compartment %s, error: %v", ad, sc.CompartmentId, err)
			}
			log.Info("AD is not provided in storage class and determined from Requisite topology.")
			sc.AvailabilityDomain = *fullAd.Name
		}
	}
	// If still empty, fallback to first AD in region (default to first AD available)
	if sc.AvailabilityDomain == "" {
		log.Info("AD is not provided in storage class and not determined from topology, picking default from ListAvailabilityDomains response.")
		ads, err := identityClient.ListAvailabilityDomains(ctx, sc.CompartmentId)
		if err != nil || len(ads) == 0 {
			log.With("service", "identity", "verb", "list", "resource", "availabilityDomain", "statusCode", util.GetHttpStatusCode(err)).
				With(zap.Error(err)).Error("Failed to resolve availability domains for compartment.")
			metricDimensions[metrics.ComponentDimension] = util.GetComponentForMetricDimension(util.GetError(err), util.CSIStorageType)
			metrics.SendMetricData(d.metricPusher, metrics.LustreProvision, time.Since(startTime).Seconds(), metricDimensions)
			return nil, status.Errorf(codes.InvalidArgument, "Failed to resolve availability domains for compartment %s: %v", sc.CompartmentId, err)
		}
		sc.AvailabilityDomain = *ads[0].Name
	}
	log = log.With("availabilityDomain", sc.AvailabilityDomain)
	...
	if len(existingLustreFileSystems) == 1 {
		...
		default:
			...
			if existingLustreFs.LifecycleState == lustre.LustreFileSystemLifecycleStateFailed {
				...
				defer func() {
					// do not let best-effort diagnostics break the flow
					_ = recover()
				}()
	...
	_, err = lustreClient.AwaitLustreFileSystemActive(ctx, log, *lustreFs.Id)
	if err != nil {
...
func (d *LustreControllerDriver) sendCreateVolumeSuccessResponse(log *zap.SugaredLogger, lustreFs *lustre.LustreFileSystem, metricDimensions map[string]string, startTime time.Time, sc *LustreStorageClassParameters) (*csi.CreateVolumeResponse, error) {
	volumeHandle := buildLustreVolumeHandle(lustreFs) //TODO Validate this, else it will fail in node Driver
	metricDimensions[metrics.ComponentDimension] = util.GetComponentForMetricDimension(util.Success, util.CSIStorageType)
	...
	return &csi.CreateVolumeResponse{
		Volume: &csi.Volume{
			...
			CapacityBytes: int64(*lustreFs.CapacityInGBs * client.GiB),
...
func (d *LustreControllerDriver) DeleteVolume(ctx context.Context, req *csi.DeleteVolumeRequest) (*csi.DeleteVolumeResponse, error) {
	...
	if lustreFilesystemId == "" {
		...
		dim[metrics.ComponentDimension] = util.GetComponentForMetricDimension(util.ErrValidation, util.CSIStorageType)
	...
	if err != nil {
		...
		dim[metrics.ComponentDimension] = util.GetComponentForMetricDimension(util.GetError(err), util.CSIStorageType)
		metrics.SendMetricData(d.metricPusher, metrics.LustreDelete, time.Since(startTime).Seconds(), dim)
	...
	dim[metrics.ComponentDimension] = util.GetComponentForMetricDimension(util.Success, util.CSIStorageType)
	metrics.SendMetricData(d.metricPusher, metrics.LustreDelete, time.Since(startTime).Seconds(), dim)
	return &csi.DeleteVolumeResponse{}, nil
}
