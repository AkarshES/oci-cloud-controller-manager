		...
		"InstantFailover": {
			...
			service: &v1.Service{
				ObjectMeta: metav1.ObjectMeta{
					...
					Annotations: map[string]string{
						ServiceAnnotationLoadBalancerType:                            NLB,
						ServiceAnnotationNetworkLoadBalancerIsInstantFailoverEnabled: "true",
					},
				},
				Spec: v1.ServiceSpec{
					IPFamilies:      []v1.IPFamily{v1.IPFamily(IPv4)},
...
func TestInstantFailover(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			svcAnnotations: map[string]string{
				ServiceAnnotationLoadBalancerType:                            NLB,
				ServiceAnnotationNetworkLoadBalancerIsInstantFailoverEnabled: "false",
			},
			expected: false,
			wantErr:  false,
		},
		...
		},
		{
			...
			svcAnnotations: map[string]string{
				ServiceAnnotationLoadBalancerType:                            NLB,
				ServiceAnnotationNetworkLoadBalancerIsInstantFailoverEnabled: "TFIknow",
			},
			expected: false,
			wantErr:  true,
		},
		{
			name: "nlb-empty-string",
			svcAnnotations: map[string]string{
				ServiceAnnotationLoadBalancerType:                            NLB,
				ServiceAnnotationNetworkLoadBalancerIsInstantFailoverEnabled: "",
			},
			expected: false,
			wantErr:  true,
		},
		{
			name: "lb-not-applicable",
			svcAnnotations: map[string]string{
				ServiceAnnotationLoadBalancerType:                            LB,
				ServiceAnnotationNetworkLoadBalancerIsInstantFailoverEnabled: "true",
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := getInstantFailover(svc)
			if (err != nil) != tt.wantErr {
				t.Errorf("getInstantFailover() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if (got != nil) && *got != tt.expected {
				t.Errorf("getInstantFailover = %v, expected %v", *got, tt.expected)
			}
		})
	}
}
