...
func (f *Framework) GetOrImportCertificate() string {
	// try to find a certificate that can be reused for E2E test
	listRequest := certificatesmanagement.ListCertificatesRequest{
		CompartmentId:                common.String(f.Compartment1),
		LifecycleState:               certificatesmanagement.ListCertificatesLifecycleStateActive,
		IssuerCertificateAuthorityId: nil,
	}
	if ls, err := f.certificateClient.ListCertificates(context.Background(), listRequest); err != nil {
		Logf(fmt.Sprintf("no exisitng certificate found due to error %s", err))
	} else if len(ls.Items) > 0 {
	...
	// Import a new certificate into Certificate Management Service
	cert, key, chain, err := CreateCertificateChain()
	if err != nil {
		return ""
	}
	...
	if certificate, err := f.certificateClient.CreateCertificate(context.Background(), request); err != nil {
		Logf(fmt.Sprintf("Unable to create certificate due to error %s with message %s", err, err.Error()))
		return ""
	} else {
		// Wait for certificate to be available
...
func (f *Framework) DeleteCertificate(certId *string) {
...
// DeleteImportedCertificate all the imported certificates generated by code
func (f *Framework) DeleteImportedCertificate(lifecycleState string) {
	// try to find a certificate that can be reused for E2E test
	if stateEnum, ok := certificatesmanagement.GetMappingListCertificatesLifecycleStateEnum(lifecycleState); !ok {
		Logf("invalid lifecycle state: %s", lifecycleState)
	} else {
		listRequest := certificatesmanagement.ListCertificatesRequest{
			CompartmentId:                common.String(f.Compartment1),
			LifecycleState:               stateEnum,
			IssuerCertificateAuthorityId: nil,
		}
		if certs, err := f.certificateClient.ListCertificates(context.Background(), listRequest); err != nil {
		} else {
			for _, cert := range certs.Items {
				f.DeleteCertificate(cert.Id)
			}
		}
	}
}
...
func CreateCertificateChain() (certificatePem *string, certificateKey *string, certificateChain *string, err error) {
	...
	cert := &x509.Certificate{
		...
		Subject: pkix.Name{
			Organization: []string{"Company, INC."},
			CommonName:   "Company, INC",
		},
		IPAddresses:  []net.IP{net.IPv4zero, net.IPv6loopback},
		NotBefore:    time.Now(),
	...
	certPrivKeyPEM := new(strings.Builder)
	...
	// Assign the fields
	certificatePemStr := certPEM.String()
	certificateKeyStr := certPrivKeyPEM.String()
	certificateChainStr := certPEM.String() + caPEM.String()
	certificateKey = &certificateKeyStr
	certificatePem = &certificatePemStr
	certificateChain = &certificateChainStr
	return
}
