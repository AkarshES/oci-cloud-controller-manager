...
var _ = Describe("Pre Upgrade testing", func() {
	...
	Context("[pre-upgrade]", func() {
		It("Create statefulset with PVC and POD for CSI.", func() {
			...
			scName := f.CreateStorageClassOrFail(framework.ClassOCICSI, setupF.BlockProvisionerName, nil, pvcJig.Labels, "WaitForFirstConsumer", false, "Delete", nil)
			serviceName := pvcJig.CreateService(setupF.UpgradeTestingNamespace)
	...
	Context("[pre-upgrade]", func() {
		It("Create PVC with fstype as XFS", func() {
			...
			scName := f.CreateStorageClassOrFail(framework.ClassOCIXfs, setupF.BlockProvisionerName, map[string]string{framework.FstypeKey: "xfs"}, pvcJig.Labels, "WaitForFirstConsumer", true, "Delete", nil)
			serviceName := pvcJig.CreateService(setupF.UpgradeTestingNamespace)
		...
		It("Create PVC with fstype as EXT3", func() {
			...
			scName := f.CreateStorageClassOrFail(framework.ClassOCIExt3, setupF.BlockProvisionerName, map[string]string{framework.FstypeKey: "ext3"}, pvcJig.Labels, "WaitForFirstConsumer", true, "Delete", nil)
			serviceName := pvcJig.CreateService(setupF.UpgradeTestingNamespace)
			pvcJig.CreateAndAwaitStatefulSetCSI("csi-app-oci-bv", serviceName, scName, framework.MaxVolumeBlock, replicas, v1.PersistentVolumeFilesystem, v1.ReadWriteOnce)
		})
	})
