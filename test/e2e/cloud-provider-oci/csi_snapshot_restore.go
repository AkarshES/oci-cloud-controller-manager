...
var _ = Describe("Snapshot Creation and Restore", func() {
	...
	Context("[cloudprovider][storage][csi][snapshot][restore]", func() {
		...
		It("Should be able to create and restore a snapshot from a backup(static case)", func() {
			...
			pvcRestore := pvcJig.CreateAndAwaitPVCOrFailSnapshotSource(f.Namespace.Name, framework.MinVolumeBlock, scName, restoreVsName, v1.ClaimPending, nil)
...
func testSnapshotAndRestore(f *framework.CloudProviderFramework, scParams map[string]string, vscParams map[string]string) {
	checkOrInstallCRDs(f)
	pvcJig := framework.NewPVCTestJig(f.ClientSet, "csi-snapshot-restore-e2e-tests")
	pvcJig.InitialiseSnapClient(f.SnapClientSet)
...
func checkOrInstallCRDs(f *framework.CloudProviderFramework) {
	...
	if err != nil {
		if setupF.EnableCreateCluster == false{
			...
			framework.RunKubectl("create", "-f", "https://raw.githubusercontent.com/kubernetes-csi/external-snapshotter/v6.1.0/client/config/crd/snapshot.storage.k8s.io_volumesnapshots.yaml")
		}
	}
	...
	if err != nil {
		if setupF.EnableCreateCluster == false{
			...
			framework.RunKubectl("create", "-f", "https://raw.githubusercontent.com/kubernetes-csi/external-snapshotter/v6.1.0/client/config/crd/snapshot.storage.k8s.io_volumesnapshotclasses.yaml")
		}
	}
	...
	if err != nil {
		if setupF.EnableCreateCluster == false{
			...
			framework.RunKubectl("create", "-f", "https://raw.githubusercontent.com/kubernetes-csi/external-snapshotter/v6.1.0/client/config/crd/snapshot.storage.k8s.io_volumesnapshotcontents.yaml")
		}
	}
}
