...
locals {
  ...
  enable_validation = var.cpo-image-validation-enabled && (length(data.odo_applications.image-release-validator-ccm-csi.applications) > 0) && (local.artifact_versions["image-release-validator-hyperkube"].version == "skip")
}
