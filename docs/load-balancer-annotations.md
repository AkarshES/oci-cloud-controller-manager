...
| `service.beta.kubernetes.io/oci-load-balancer-backend-protocol`              | Specifies protocol on which the listener accepts connection requests. Supported values are `TCP`, `HTTP`, `GRPC`, and `HTTP2` (case-insensitive). `GRPC` and `HTTP2` require listener TLS configuration; otherwise listener creation is rejected during validation.                | `"TCP"`                                          |
...
- Invalid `oci-load-balancer-backend-protocol` values are rejected. Valid values are `TCP`, `HTTP`, `GRPC`, and `HTTP2`.
## TLS-related
...
| `oci-network-load-balancer.oraclecloud.com/is-instant-failover-enabled`    | Enable to redirect existing traffic to a healthy backend server if the current backend server becomes unhealthy                                            | `false`                                   |
