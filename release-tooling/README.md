...
2. Update commit ID and artifact information (against the "artifacts" and "resolver_params" keys) in release-tooling/templates/cpo-release/meta.py

**Notes:** 
1. Sheepy requires us to use the same commit ID for application and infra releases
...
```
cd release-tooling
sheepy init -t templates/cpo-release/meta.py
```

# Create Releases

The above should create a file in release-tooling/releases/cpo-release folder. Execute the below replacing the filename with the newly created file

```
sheepy deploy -d releases/cpo-release/<created-file-name>.json create --all
```

# Create CM

```
sheepy cm -d releases/cpo-release/<created-file-name>.json create
```
