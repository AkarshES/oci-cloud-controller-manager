...
func (r *NodeOperationRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if updatedInProgressNodes != nil && len(updatedInProgressNodes) != 0 {
		...
		for _, inProgressNode := range updatedInProgressNodesCopy {
			nodeOperationUpdate := r.getWorkRequest(ctx, nor.Name, inProgressNode.WorkRequestId, inProgressNode.NodeName)
			...
			case UpdateTypeSucceeded:
				...
				if err != nil {
					...
					if latency != 0 {
						nodeDimension := map[string]string{norNodeName: inProgressNode.NodeName}
...
func (r *NodeOperationRuleReconciler) triggerNodeAction(ctx context.Context, node *v1.Node, nor norv1beta1.NodeOperationRule) (string, NodeOperationFailure) {
	...
	if &nor.Spec != nil && &nor.Spec.Actions != nil && len(nor.Spec.Actions) == 1 {
		...
		if err != nil {
			nodeOperationFailure := r.convertApiErrorToNodeOperationFailureType(ctx, node.Name, err, "")
...
func (r *NodeOperationRuleReconciler) convertApiErrorToNodeOperationFailureType(ctx context.Context, nodeName string, err error, workRequestId string) NodeOperationFailure {
	logger := log.FromContext(ctx, norNodeName, nodeName, norWorkRequestId, workRequestId)
	...
	if err != nil {
		...
		if errors.As(err, &ociServiceError) {
			...
			case HTTP404NotAuthorizedOrNotFoundCode:
				// TODO: invalid cluster id / node id -> need to send metrics and alarm because there might be a bug in operator
				// https://dyn.slack.com/archives/C073EMW8E30/p1730769071221349?thread_ts=1730742868.854419&cid=C073EMW8E30
				// TODO: if it is missing system policy from customer to non-prebake image SMN, need to populate clear k8s events
...
func (r *NodeOperationRuleReconciler) getWorkRequest(ctx context.Context, norCRName string, workRequestId string, nodeName string) NodeOperationResultUpdate {
	logger := log.FromContext(ctx, norNodeName, nodeName, norWorkRequestId, workRequestId, norName, norCRName)
	...
	case containerengine.WorkRequestStatusFailed:
		nodeOperationFailure := r.convertApiErrorToNodeOperationFailureType(ctx, nodeName, err, workRequestId)
