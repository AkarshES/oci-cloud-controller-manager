...
* If the release fails in `Pulling Image` stage, check the error message and response code for that stage.
  * If you see the following error, the ODO app did not have sufficient permissions to pull the image.
    ```
    Error returned by ObjectStorage Service. Http Status Code: 404. Error Code: BadErrorResponse. Opc request id: iad-1:yHN2CwbeMWXZXjlxjA8Rpo0qHPdZgIRN_Ei3TN5P11i3cM_zFSip0Dk6zu-udVkh. Message: Failed to parse json from response body due to: unexpected end of JSON input. With response body .
